...
class OptimizePress_Page_Options {
	...
	function load_layout($type='body',$array=false,$id='',$class='',$default=array(),$one_col=false){
		...
		$entry = $wpdb->get_var( $wpdb->prepare(
			"SELECT layout FROM `{$wpdb->prefix}optimizepress_post_layouts` WHERE `post_id` = %d AND `type` = %s",
			$this->_page_id,
			$type
		));
		$layout = $default;
	...
	function generate_layout($layout,$type,$one_col=false)
	{
		...
		if(defined('OP_LIVEEDITOR')){
			...
			$row_start = ($one_col?'':'<div class="op-row-links"><a title="'.__('Edit Row Options', OP_SN).'" href="#options" class="edit-row" id="row_options"></a><a title="'.__('Clone Row', OP_SN).'" href="#clone-row" class="clone-row"></a><a title="'.__('Move Row', OP_SN).'" href="#move" class="move-row"></a><a href="#add-new-row" class="add-new-row"><img src="'.OP_IMG.'/live_editor/add_new.png" alt="'.__('Add New Row',OP_SN).'" /><span>'.__('Add New Row',OP_SN).'</span></a><a title="'.__('Delete Row', OP_SN).'" href="#delete-row" class="delete-row"></a></div>');
			$row_end = '';
	...
	function update_layout($layout,$type='body'){
		...
		if(!$entry){
			...
			$wpdb->update($table,array('layout'=>$layout),array('id'=>$entry));
		}
	}
