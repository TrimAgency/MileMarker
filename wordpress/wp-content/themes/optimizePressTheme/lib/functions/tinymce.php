	...
	public static function editor_settings($editor_id, $set) {
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty(self::$first_init) ) {
				...
				//Check if we are running Firefox. If not, load the plugins
				//if (strlen(strstr($_SERVER['HTTP_USER_AGENT'], "Firefox"))==0) $plugins = array( 'inlinepopups', 'spellchecker', 'tabfocus', 'paste', 'media', 'fullscreen', 'wordpress', 'wpeditimage', 'wpgallery', 'wplink', 'wpdialogs' );
...
<?php
		...
		if ( $tmce_on ) {
			...
			if ( 'en' != self::$mce_locale && isset($lang) ) {
				...
				//wp_enqueue_script(OP_SN.'-wp-langs-en', $baseurl.'/langs/wp-langs-en.js?'.$version, array(OP_SN.'-noconflict-js'), OP_VERSION);
			}
		}
?>
