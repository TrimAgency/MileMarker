	...
	function parse_shortcode(){
		...
		if(preg_match('/'.op_shortcode_regex('op_liveeditor_elements').'/s',$sc,$matches)){
			if($GLOBALS['OP_LIVEEDITOR_DEPTH'] === 1){
				...
				$processed = '<h1>'.__('Content LiveEditor',OP_SN).'</h1><div class="epicbox-actual-content">'.$processed.'<a href="#add_element" class="add-new-element"><img src="'.OP_IMG.'/live_editor/add_new.png" alt="'.esc_attr__('Add Element',OP_SN).'"><span>'.__('Add Element',OP_SN).'</span></a></div>
				...
				$child_html = op_process_asset_content(apply_filters('the_content',$child_data['liveeditorElements'])).'<div class="op-hidden"><textarea class="op-le-child-shortcode" name="shortcode[]">'.op_attr(shortcode_unautop($matches[0])).'</textarea></div>';
				/*
				 * $ needs to be escaped
				 */
...
';
		...
		$zip->close();
		$out = array('output'=>'<a href="'.OP_LIB_URL.'content_layouts/export/'.$tmpdirname.'/'.$filename.'.zip">'.__('Download your layout',OP_SN).'</a> | <a href="#delete" class="delete-file">'.__('Delete File',OP_SN).'</a><input type="hidden" name="zip_filename" id="zip_filename" value="'.$full_dir.'/'.$filename.'.zip" />');
