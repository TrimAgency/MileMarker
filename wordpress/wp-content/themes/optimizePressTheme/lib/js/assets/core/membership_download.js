	...
	return {
		...
		customInsert: function(attrs){
			...
			for(var i in attrs.elements){
				...
				str += '[download title="'+q.replace( /"/ig,"'")+'" '+field+'="'+ic+'" icon_folder="' + op_asset_settings.attributes.step_2.elements.multirow.attributes.icon.asset[1] + '" file="'+file+'" package="'+pack+'" level="'+level+'" new_window="' +  new_window + '"]'+a+'[/download]';
			};
