;(function($){
	var container, slides = [], slide_content = [], slide_widths = [], slides_ul, waiting,
		...
		wp_post = false,
		current_config = {},
		current_picker, farbtastic, picker,
		folders = {},
		show_ons = {},
		stored_selectors = {},
		stored_elements = {},
	...
	$(document).ready(function(){
		...
		var asset_list = slide_content[1].find('div.asset-list'), no_asset_elem = $('#op_asset_browser_no_assets');
		$('#op_assets_filter').keyup(function(e){
			no_asset_elem.hide();
			var v = $.trim($(this).val()), items = asset_list.find('li');
			if(!v || v == ''){
				asset_list.show();
				items.filter(':not(:visible)').fadeIn('fast');
			} else {
				v = v.toLowerCase();
				items.find('span.title').each(function(){
					var e = $(this), t = e.text().toLowerCase(), show = (t.indexOf(v) > -1),
						add_class = 'hidden', remove_class = 'visible', func = 'fadeOut';
					if(show){
						add_class = 'visible';
						remove_class = 'hidden';
						func = 'fadeIn';
					}
					e.closest('li').filter((show?':not(:visible)':':visible')).addClass(add_class).removeClass(remove_class)[func]('fast');
				});
				asset_list.each(function(){
					$(this)[($(this).find('li.visible').length > 0 ? 'show' : 'hide')]();
				});
				if(asset_list.filter(':visible').length < 1){
					no_asset_elem.show();
				}
			}
	...
	function load_config(h,callback){
		callback = callback || function(){};
		if(op_assets[h[0]][h[1]].settings == 'Y'){
			if(typeof op_stored_configs[h[0]] == 'undefined'){
				op_stored_configs[h[0]] = {};
				op_stored_steps[h[0]] = {};
			}
			if(typeof op_stored_configs[h[0]][h[1]] == 'undefined'){
				var url = OptimizePress[h[0]+'_assets_url']
				if(h[0] != 'core'){
					url += h[1]+'/';
				}
				$.getScript(url+h[1]+'.js',function(){
					setup_settings_panel(h[0],h[1],op_asset_settings);
					if($.object_length(folders) > 0){
						$.post(OptimizePress.ajaxurl,
							...
							function(resp){
								for(var type in resp){
									for(var field in resp[type]){
										...
										if(type == 'image' || type == 'style'){
											...
											el.removeClass('loading-asset-dropdown');
								...
								show_settings(h[0],h[1]);
								callback(op_stored_steps[h[0]][h[1]],op_stored_configs[h[0]][h[1]]);
				...
				if(typeof op_stored_steps[h[0]][h[1]] != 'undefined'){
					$.each(op_stored_steps[h[0]][h[1]],function(){
		...
		if($.inArray(type,['h1','h2','h3','h4','h5','h6','p']) > -1){
			...
			switch(type){
				case 'custom_html':
					...
					break;
				case 'button_preview':
					str.append('<div class="preview_border preview-wrapper"><div class="preview-outer"><div class="preview-inner preview_border op-asset-dropdown" id="' + tmp_id + '_container"><a href="#" class="selected-item css-button style-1 pbox_' + field.addClass + '" id="op_button_preview">'
							...
							if ((panel == true && $('#panel_control_' + idprefix + k).is(':checked')) || panel == false) {
								...
								for (var l in attributes[i][j].elements[k].attributes) {
									...
									if (type == 'text_properties') {
										...
										attrs[l + '_bold'] = $('.field-id-' + idprefix+k+'_'+l + ' input[name="op_font[style_checkbox_text][bold]"]').is(':checked') ? 'Y' : '';
										attrs[l + '_italic'] = $('.field-id-' + idprefix+k+'_'+l + ' input[name="op_font[style_checkbox_text][italic]"]').is(':checked') ? 'Y' : '';
										attrs[l + '_underline'] = $('.field-id-' + idprefix+k+'_'+l + ' input[name="op_font[style_checkbox_text][underline]"]').is(':checked') ? 'Y' : '';
	...
	function add_wysiwyg(id,content){
		...
		if(use_wysiwyg){
			...
			$('#' + id).css({ 'display': 'block', 'height': '200px' });
	...
	function edit_element(settings){
		var h = settings.asset;
		container.addClass('edit-mode');
		disable_slide = true;
		disable_insert = true;
		if(typeof op_assets[h[0]] != 'undefined' && typeof op_assets[h[0]][h[1]] != 'undefined'){
			set_titles(slides[1].find('a[href$="#'+h[0]+'/'+h[1]+'"]'));
			container.find('.op_asset_browser_slide .settings-container').css('display','none');
			load_config(h,function(steps,config){
	...
	*/
			...
			afterShow: function () {
				...
				if ($fancyBoxIframe.length > 0) {
					...
					if ($assetBrowserContainer.length > 0) {
						$fancyBoxOpened
							.find('.op_asset_browser_slide').eq(slide)
							.find('.op_asset_content')
								.addClass('op_no_outline')
								.attr('tabindex', 0)
								.focus();
	...
	function generate_video_link(vid){
		...
		return '<a href="'+OptimizePress.flowplayer+'?config='+opts+'" title="'+translate('help_video')+'" class="help-me"><span>?</span></a> \
<div> \
