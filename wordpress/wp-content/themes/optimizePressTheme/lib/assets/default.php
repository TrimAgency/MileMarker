...
class OptimizePress_Default_Assets {
	...
	function _add_element_field($atts,$content,$tag='',$prepend='',$append=''){
		...
		if(defined('OP_PAGEBUILDER')){
			if($GLOBALS['OP_LIVEEDITOR_DEPTH'] == 0){
				...
				$new_content = $content;
				if(!preg_match('/'.op_shortcode_regex('op_liveeditor_elements').'/s',$content,$matches)){
	...
	*/
	function parse_list($assets){
		...
		$tags = array(
			'button' => array('button_1','button_2','button_3','button_4','button_5','button_6','button_cart'),
	...
	*/
	function affiliate_page($atts,$content=''){
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function bullet_block($atts,$content='')
	{
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function button_2($atts,$content=''){
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function button_3($atts,$content=''){
		//Generate an ID for this button
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function button_4($atts,$content=''){
		//Generate an ID for this button
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function content_toggle($atts,$content=''){
		//Init variables
	...
	* 	$atts: Contains all the attributes for this asset
	*/
	function countdown_cookie_timer($atts){
		//Get the attributes from the shortcode
		$data = shortcode_atts(array(
			'style' => 1,
			'length_of_time' => '',
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function delayed_content($atts,$content=''){
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function feature_block($atts,$content=''){
		//Extract the attributes into variables
		extract(shortcode_atts(array(
			'style' => 'icon',
			'columns' => '2',
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function feature_box_creator($atts, $content=''){
		//Extract the attributes into variables
		$data = shortcode_atts(array(
			'style' => '1',
			'letter_spacing' => '',
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function guarantee_content($atts,$content){
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function guarantee_box($atts,$content=''){
		//Extract the attributes into variables
		extract(shortcode_atts(array(
			'style' => '1',
			'title' => '',
	...
	* 	$content: Contains what is inside the shortcode tags
	*/
	function headline($atts,$content=''){
		//Extract the attributes into variables
		extract(shortcode_atts(array(
			'style' => '1',
			'align' => 'left',
	...
	function hyperlink($atts,$content=''){
		extract(shortcode_atts(array(
			'href' => '',
			'align' => 'left',
			'new_window' => 'N',
	...
	function images($atts)
	{
		$uid = 'imgs_' .md5(serialize($atts));
		...
		/*
		 * We need to trigger pretty photo if it hasn't been already triggered
		 */
		if (false === self::$add_pretty_photo && isset($data['pretty_photo'])) {
			self::$add_pretty_photo = $data['pretty_photo'];
		}

		return $data['output'];
	}

	function img_alert($atts,$content=''){
		extract(shortcode_atts(array(
			'image' => '',
			'text' => '',
		), $atts));
	...
	function img_text_aside($atts,$content=''){
		$data = shortcode_atts(array(
			'style' => 1,
			'image' => '',
	...
	function news_bar($atts,$content=''){
	...
	function optin_box($atts,$content='')
	{
		$uidOpBox = 'optbox_' . md5(serialize($atts) . $content . (int)defined('OP_LIVEEDITOR'));

		if (false === $template = get_transient('el_' . $uidOpBox)) {
			$oldAtts = $atts;
			$atts = shortcode_atts(array(
			...
			foreach($extra_fields as $name => $value){
				$data['extra_fields'][$name] = '<div class="text-box"><input type="text"' . $required . ' name="'.$name.'" placeholder="'.$value.'" value="' . getOptinUrlValue($name) . '" /></div>';
			}
			...
			foreach($fields as $name => $info){
				$input_container_start_html = ($style!=12 && $style!=15 && $style!=19 && $style!=20 && $style!=21 && $style!=22 && $style!=23 ? '<div class="text-box '.$name.'">' : '');
				$input_container_end_html = ($style!=12 && $style!=15 && $style!=19 && $style!=20 && $style!=21 && $style!=22 && $style!=23 ? '</div>' : '');
				if ($name === 'email') {
					$data['fields'][$name.'_field'] = $input_container_start_html.'<input type="email"' . $required . ' name="'.$info['name'].'"'.($info['text']==''?'':' placeholder="'.$info['text'].'"').' value="' . getOptinUrlValue($info['name']) . '" />'.$input_container_end_html;
				} else {
					$data['fields'][$name.'_field'] = $input_container_start_html.'<input type="text"' . $required . ' name="'.$info['name'].'"'.($info['text']==''?'':' placeholder="'.$info['text'].'"').' value="' . getOptinUrlValue($info['name']) . '" />'.$input_container_end_html;
				}
			}
			...
			set_transient('el_' . $uidOpBox, $template, OP_SL_ELEMENT_CACHE_LIFETIME);
		}
		...
		return str_replace('%%redirect_url%%', op_current_url(), $template);
	}
	...
	function order_box($atts,$content=''){
		$data = shortcode_atts(array(
			'style' => 1,
			'title' => '',
			'header' => '',
	...
	function order_step_graphics($atts)
	{
	...
	function pricing_element($atts,$content=''){
		$content = op_clean_shortcode_content($content);
	...
	function qna_elements($atts,$content=''){
		extract(shortcode_atts(array(
			'style' => 'qa-text',
			'columns' => '2',
		), $atts));
	...
	function question($atts,$content='',$tag=''){
		extract(shortcode_atts(array(
			'question' => '',
		), $atts));
		$style = 'qa-text';
...
<li>
	...
	function step_graphics($atts,$content=''){
		//Initialize variables
		$data = shortcode_atts(array(
			'style' => 1,
			'color' => '',
...
<div class="tab-content">'.$content.'</div>';
	...
	function terms_conditions($atts,$content=''){
	...
	function two_column_block($atts,$content=''){
...
<div class="double-column cf">
	...
	function vertical_spacing($atts){
		extract(shortcode_atts(array(
			'height' => '20'
		),$atts));
	...
	function video_lightbox($atts,$content=''){
	 ...
	 * @return String
	 */
	function file_download($atts, $content)
	{
		extract(shortcode_atts(array(
			'style' => 1,
		), $atts));
	 ...
	 * @return String
	 */
	function membership_download($atts, $content)
	{
		extract(shortcode_atts(array(
			'style' => 1,
		), $atts));
	...
	function download($atts, $content)
	{
		...
		if ($protected) {
			if (!$amazon) {
				$fileLink = site_url('?optimizemember_file_download='). $forUrl . '/' .  $onlyFileName;
			} else {
				$fileLink = site_url('?optimizemember_file_download='). '/' .  $onlyFileName;
			}
		} else {
			$fileLink = $file;
		}
	 ...
	 * @return string
	 */
	function membership_login_form($atts)
	{
		// var_dump($atts);
		extract(shortcode_atts(array(
			'style'						=> '1',
	 ...
	 * @return String
	 */
	function javascript($atts, $content='')
	{
	 ...
	 * @return String
	 */
	function membership_order_button($atts, $content)
	{
	...
	function _print_front_scripts($return=false){
...
';
	...
	function membership_array($type, $selected_id=0, $parent_id=0) {
		global $wpdb;
		$temp[] = '\'\':\'\'';
		$t = array();
